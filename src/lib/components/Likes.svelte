<section class="border-t border-gray-800 pt-12">
	<div class="space-y-8">
		<h2 class="text-center text-2xl font-semibold text-white">
			<code class="rounded border border-gray-700 bg-gray-900 px-3 py-1 text-green-400"
				>grep -r "interests" ~/life/</code
			>
		</h2>

		<div
			class="mx-auto prose prose-lg max-w-4xl prose-invert prose-h1:font-mono prose-h1:text-xl prose-h1:text-cyan-400 prose-ul:text-gray-300"
		>
			<h1>[#] Food</h1>
			<ul>
				<li>Ramen</li>
				<li>Middle Eastern</li>
				<li>Coffee</li>
				<li>Dark Chocolate</li>
				<li>Sushi</li>
			</ul>

			<h1>[*] Sports</h1>
			<ul>
				<li>Tennis</li>
				<li>Swimming</li>
				<li>Cycling</li>
				<li>Chess</li>
				<li>Hiking</li>
			</ul>

			<h1>[+] Tech</h1>
			<ul>
				<li>Mechanical Keyboards</li>
				<li>Linux Ricing</li>
				<li>Self-hosting</li>
				<li>Open Source</li>
				<li>Vim</li>
			</ul>

			<h1>[~] Media</h1>
			<ul>
				<li>Sci-fi Movies</li>
				<li>Tech Documentaries</li>
				<li>Podcasts</li>
				<li>Audiobooks</li>
				<li>Anime</li>
			</ul>

			<h1>[&] Hobbies</h1>
			<ul>
				<li>Photography</li>
				<li>Music Production</li>
				<li>Gaming</li>
				<li>Reading</li>
				<li>Cooking</li>
			</ul>
		</div>
	</div>
</section>
